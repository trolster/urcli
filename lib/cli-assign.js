    ...
    if (assignedCount < maxSubmissions) {
      ...
      .then(res => {
        ...
        if (!submissionRequest) {
          ...
          // Update the prompt if the request has been updated
          checkUpdate(submissionRequest.updated_at)
...
function checkUpdate (updatedAt) {
  let timeSinceUpdate = Date.now() - Date.parse(updatedAt)
  console.log(timeSinceUpdate)
  // If it was upated in the past 30 seconds we get the positions.
