    ...
    if (assignedCount < 2) {
      ...
      .then(res => {
        ...
        if (!submissionRequest) {
          ...
          let needToUpdate = checkIfProjectIdsAreEqual(submissionRequest)
...
function checkIfProjectIdsAreEqual (submissionRequest) {
...
function setPrompt () {
  ...
  positions.forEach(project => {
    const pid = `id: ${chalk.white.dim(project.project_id)}`
    const pname = `name: ${chalk.white.dim(config.certs[project.project_id].name)}`
    const plang = `lang: ${chalk.white.dim(project.language)}`
    const ppos = `queue pos: ${chalk.white(project.position)}`
    console.log(chalk.blue(`    ${pid} - ${pname} - ${plang} - ${ppos}\n`))
  })
  if (!positions.length) {
