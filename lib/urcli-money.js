...
class Report {
  ...
  countReview (review) {
    ...
    let turnaround_time = completed_at.diff(assigned_at, 'hours', true)
    ...
    if (!this.projects.hasOwnProperty(id)) {
      ...
      this.totalTurnaroundTime += turnaround_time
  ...
  print () {
    ...
    for (let p in this.projects) {
      let {name, id, ungradeable, passed, failed, earned, turnaround_time} = 
          this.projects[p]
      console.log(`
      ${chalk.white(`Project: ${name} (${id}):`)}
          ...
          ${chalk.white(`Turnaround Time: ${turnaround_time.toFixed(2)}`)}
          ${chalk.white(`Average Turnaround Time: ` +
                  `${(turnaround_time/(passed+failed)).toFixed(2)}`)}
          `)
    }
    console.log(chalk.bgBlack.white(`Total Reviewed: ${this.totalReviewed}`))
    console.log(chalk.bgBlack.white(`Total Earned: ${this.totalEarned}`))
    console.log(chalk.bgBlack.white(`Total Turnaround Time: ` +
                `${this.totalTurnaroundTime.toFixed(2)}`))
    console.log(chalk.bgBlack.white(`Average Turnaround Time: ` +
                `${(this.totalTurnaroundTime/this.totalReviewed).toFixed(2)}`))
    console.log('========================================')
  }
}
