  ...
  print () {
    ...
    for (let p in this.projects) {
      let {name, id, ungradeable, passed, failed, earned, turnaround_time} = 
          this.projects[p]
      let avg_turnaround_time = moment.utc(turnaround_time/(passed+failed))
      ...
      ${chalk.white(`Project: ${name} (${id}):`)}
          ...
          ${chalk.white(`Average Turnaround Time: ` +
                                      `${avg_turnaround_time.format('HH:mm')}`)}
          `)
    }
    console.log(chalk.bgBlack.white(`Total Assigned: ${this.totalAssigned}`))
